...
- Planner: [#265](https://github.com/dearlordylord/huly-mcp/issues/265) adds native document-attached ToDos. Team scheduled-work/free-busy reporting remains a derived, privacy-blocked projection; `TodoAutomationHelper` is excluded server plumbing and capacity is not defined by the current model.
- Recruiting: [#264](https://github.com/dearlordylord/huly-mcp/issues/264) adds native-metadata Candidate custom fields. New-person Candidate enablement remains the explicit `create_person` then `set_recruiting_candidate_profile` workflow.
- Calendar and virtual office: [#266](https://github.com/dearlordylord/huly-mcp/issues/266) covers caller Calendar settings; [#267](https://github.com/dearlordylord/huly-mcp/issues/267) covers bounded Floor/Room administration; [#268](https://github.com/dearlordylord/huly-mcp/issues/268) covers meeting-room composition. Provider-owned, transient, transcript, recording, and RSVP behavior is excluded or blocked.
...
- Recruiting: vacancies, candidates, applicants, matches, reviews, opinions, skills, comments, attachments, activity, and related issues are covered. #264 tracks friendly custom fields; the two-call candidate-create workflow is the decided policy.
...
- Calendar: #266 tracks caller-scoped visibility/hidden settings and PrimaryCalendar preference. Calendar creation/deletion, provider-owned ExternalCalendar data, disconnect, and RSVP are not part of this surface.
- Team planner and schedule reporting: privacy-safe scheduled-work/free-busy projection remains deferred; capacity is not inferred from meeting availability.
- Virtual office and meetings: #267 and #268 track durable Floor/Room administration and room-aware Event/Schedule composition. Raw transient occupancy, LiveKit sessions/devices, transcription, recording, and server-owned MeetingMinutes lifecycle are excluded or blocked.
