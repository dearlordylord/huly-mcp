...
| `invite_employee` | Invite a new employee by exact email, or resend an invitation to an existing inactive Employee resolved by exact email or exact display name. Active employees and existing non-Employee persons are rejected. Returns no invitation link, credential, or token. |
