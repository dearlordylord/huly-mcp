...
| Tool | Description |
|------|-------------|
| `list_card_comments` | List comments on a Huly card. Accepts card space name/ID and card title/ID. Returns comments sorted by creation date (oldest first). |
| `add_card_comment` | Add a comment to a Huly card. Comment body supports markdown formatting. Markdown links to current-workspace Huly browse URLs with _class, _id, and label become native Huly references; external URLs and other-workspace browse URLs stay normal links. |
| `update_card_comment` | Update an existing comment on a Huly card. Comment body supports markdown formatting. Markdown links to current-workspace Huly browse URLs with _class, _id, and label become native Huly references; external URLs and other-workspace browse URLs stay normal links. |
| `delete_card_comment` | Delete a comment from a Huly card. This action cannot be undone. |
