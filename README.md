...
| `list_employees` | List employees (persons who are team members), sorted by modification date (newest first). Each summary exposes stable Contact Employee fields city, email, role, statuses count, personUuid, position, active, and modifiedOn. Avatar refs/properties are provider/blob metadata; attachment/comment/channel counters and social identity collections are derived; birthday/profile need separate contracts; audit-by and internal class/space refs are intentionally unsupported in this projection. |
