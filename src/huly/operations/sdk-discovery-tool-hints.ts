...
const contactCoveredRationale =
  "Current contacts tools expose person, organization, employee/member, channels, exact-locator person notes and attachments, person administration, explicit source/survivor person merges with preview tokens and model-metadata-driven native reference migration, and employee invitation/deactivation lifecycle administration. Person administration exposes avatar and birthday, derives contact/workspace status, social identities and channel activity, reads account profile data, and applies only Huly-native account-authoritative SocialIdentity projection repairs; arbitrary identity mutations remain explicitly unsupported. Employee summaries project stable Contact Employee fields including active, role, statuses, position, and personUuid. Lifecycle tools distinguish Person, Employee.active, account linkage, and workspace membership; new-email invitation, inactive-employee reinvitation, guarded deactivation, and guarded workspace kick use Huly's published account and Contact contracts."
