...
const makeTelegramChannel = (id: ChannelId, value: NonEmptyString): Channel => ({
...
const modelFindAllInModel = (
  modelId: ObjectClassName,
  modelSupported: boolean
): HulyClientOperations["findAllInModel"] => {
  const modelClass: MetadataClassDoc = {
    _id: toRef<MetadataClassDoc>(modelId),
    _class: core.class.Class,
    space: core.space.Model,
    modifiedBy: actor,
    modifiedOn: 0,
    label: "fixture:string:Message",
    kind: 0
  }
  ...
  return ((classRef: unknown, query: Record<string, unknown>) =>
    Effect.succeed(
      toFindResult(modelSupported && classRef === core.class.Class && query["_id"] === modelId ? [modelClass] : [])
...
const gmailModelLayer = (modelSupported: boolean): ReturnType<typeof HulyClient.testLayer> =>
  HulyClient.testLayer({
    findAllInModel: modelFindAllInModel(ObjectClassName.make(String(gmail.class.Message)), modelSupported)
  })
...
const telegramLayer = (state: TelegramTestState): ReturnType<typeof HulyClient.testLayer> => {
  const findAllInModel = modelFindAllInModel(ObjectClassName.make(String(telegram.class.Message)), state.modelSupported)
...
describe("external channel message compatibility", () => {
  ...
  it("registers an LLM-readable no-fake-data tool contract", () => {
    ...
    expect(tool?.description).toContain("attachmentCount can temporarily lag")
    expect(tool?.description).toContain("provider delivery/read status")
  ...
  it.effect("resolves Telegram channels by exact value or stable ID and returns newest persisted messages", () =>
    Effect.gen(function* () {
      const channel = makeTelegramChannel(ChannelId.make("channel-1"), NonEmptyString.make("@ops"))
      const messages = [
        makeTelegramMessage(MessageId.make("message-old"), channel, "Old", Timestamp.make(5), false),
        makeTelegramMessage(
          MessageId.make("message-new"),
          channel,
          "Deploy complete",
          Timestamp.make(10),
          true,
    ...
    Effect.gen(function* () {
      const effect = runTelegramOperation(
        listExternalChannelMessages({ provider: "telegram", channel: ChannelIdentifier.make("@shared") }),
        {
          modelSupported: true,
          channels: [
            makeTelegramChannel(ChannelId.make("channel-1"), NonEmptyString.make("@shared")),
            makeTelegramChannel(ChannelId.make("channel-2"), NonEmptyString.make("@shared"))
          ],
          messages: []
        }
      )
