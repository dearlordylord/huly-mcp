...
| Drive | `.reference/platform/models/drive/src/index.ts`: `Drive`, `Resource`, `Folder`, `File`, `FileVersion`, `TypeFileVersion` | Only low-level `upload_file`; generic attachments can attach blobs elsewhere | Drive space CRUD, folder CRUD, file CRUD, file/folder move, path traversal, upload file into Drive, file versions, current-version selection, Drive comments/activity, Drive permissions/members |
...
| Calendar schedules and integrations | Installed `@hcengineering/calendar`; `.reference/platform/models/calendar/src/index.ts`: `Calendar`, `ExternalCalendar`, `Event`, recurring events/instances, `Schedule`, `PrimaryCalendar` | Event CRUD, recurring event create/list, instances, writable calendars | Calendar CRUD/config, external calendar sync metadata, primary calendar management, schedule objects, event participant mutations, RSVP/status if supported by current model |
...
| Virtual office / meetings | `.reference/platform/models/love/src/index.ts`: `Room`, `Office`, `Floor`, `ParticipantInfo`, `RoomInfo`, `Meeting` mixin, `MeetingMinutes`, `MeetingSchedule`; settings in `setting.class.OfficeSettings` | No office/meeting tools | Office floors/rooms CRUD, room access/language/default recording/transcription settings, meeting schedule, active participants/room info, meeting minutes/transcripts/recordings, devices preferences |
...
| `@hcengineering/calendar` | Partial | Calendar/external-calendar config, schedules, participant mutations |
