...
const validContext = {
  package: { name: "@firfi/huly-mcp", version: "0.15.0" },
  transport: { type: "http", http: { host: "127.0.0.1", port: 3000 } },
  huly: {
    url: {
      configured: true,
      valid: true,
    ...
    connectionTimeout: { configured: true, valid: true, valueMs: 30000, defaultMs: 30000, source: "env" }
  },
  auth: { method: "token", source: "env", tokenConfigured: true, emailConfigured: false, passwordConfigured: false },
  configSources: {
    env: {
      hulyUrl: true,
      hulyWorkspace: true,
  ...
  it.effect("rejects an origin that is not a parseable URL", () =>
    Effect.sync(function () {
      ...
      expect(() =>
        Schema.decodeUnknownSync(GetHulyContextResultSchema)({
          ...validContext,
          huly: { ...validContext.huly, url: { ...validContext.huly.url, origin: "not-a-valid-url" } }
        })
      ).toThrow()
    })
  )
  ...
  it.effect("rejects empty diagnostic strings", () =>
    Effect.sync(function () {
      for (const context of [
        ...
        { ...validContext, huly: { ...validContext.huly, workspace: { configured: true, value: "" } } },
        { ...validContext, toolScope: { ...validContext.toolScope, requestedTools: [""] } }
      ]) {
