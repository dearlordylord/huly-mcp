...
type ReadResourceContent = (typeof McpSchema.ReadResourceResult.Type)["contents"][number]

const textContent = (content: ReadResourceContent | undefined): string =>
...
describe("MCP resources", () => {
  ...
  it.effect("maps missing resources to final Invalid Params", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
  ...
  it.effect("maps a missing issue resource to final Invalid Params", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
